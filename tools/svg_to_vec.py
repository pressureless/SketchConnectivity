...
## Run
python3 svg_to_vec.py input.svg
'''
...
def svg2scap( path_to_svg, output_path, spacing = 2 ):
    ...
    drawing = _s.Drawing()
    for lines_s in linestrips:
        stroke = _s.Stroke(npoints=len(lines_s), has_time=False)
        stroke_x = []
        stroke_y = []
        stroke_w = []
        for cur_p in lines_s:
            stroke_x.append(cur_p[0])
            stroke_y.append(cur_p[1])
            stroke_w.append(0.5)
        stroke.x[:] = stroke_x
        stroke.y[:] = stroke_y
...
if __name__ == '__main__':
    ...
    output_path = pathlib.Path( args.path_to_svg ).with_suffix( '.vec' )
